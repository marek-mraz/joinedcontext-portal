...
pub struct Session {
    ...
    /// Unix seconds; the cookie is refused after this instant.
    pub expires_at: i64,
...
/// Writes the session into the encrypted jar.
pub fn store(jar: PrivateCookieJar, session: &Session) -> Result<PrivateCookieJar, ApiError> {
    ...
    Ok(jar.add(secure_cookie(SESSION_COOKIE, value, ttl)))
}

/// Reads the session, ignoring an expired or unparseable one.
...
/// Removes the session cookie (sign-out and every failure path).
