...
test.describe("managers", () => {
  ...
  test("adding a representation to an endpoint proposes a change", async ({ page }) => {
    ...
    await page.goto("/projects/banskabystrica/endpoints?lang=en");
    ...
    expect(writes[0]).toContain("PUT /api/v1/projects/banskabystrica/endpoints/public-air");
  ...
  test("the manager routes have no axe violations", async ({ page }) => {
    ...
    await page.goto("/projects/banskabystrica/spaces?lang=sk");
    await expect(page.getByRole("table")).toBeVisible();
    expect(await axeViolations(page)).toEqual([]);

    await page.goto("/projects/banskabystrica/endpoints?lang=sk");
    await expect(page.getByRole("table")).toBeVisible();
    expect(await axeViolations(page)).toEqual([]);
  });
});
