...
test("the assistant works in a copy and leaves both ways out to a person", async ({ browser }) => {
  ...
  try {
    ...
    // 2. The change it was asked for is in the copy, and only there: the project keeps its own
    //    value, because a copy is not applied until a person brings it back.
    await expect
      .poll(() => running(page, COPY), { timeout: 900_000, intervals: [5_000] })
      .toBe(!before);
    expect(await running(page), "the project changed while nobody had decided anything").toBe(before);

    // 3. And no Change was opened for it: inside a copy a write commits to the copy's branch, so
    //    there is nothing in Approvals yet — the person has not asked for one.
    ...
    // 5. What the assistant may do instead is show the person what the copy changes (CC-79).
    ...
    // One file changed, and its kind: the pipeline's own name is not on this page, and a text match
    // on it picks up a neighbouring resource whose name starts the same way.
